...
public class CommonUtil {
    ...
    public static int getValue(String key) {
        if (key.contains("点影响力")) {
            key = key.substring(0, key.indexOf("点影响力"));
        }
    ...
    /**
     * 将num中文转为阿拉伯数字 考虑范围: 0~9999_9999
     * 
     * @param num
     * @return
     */
    public static int convertChineseToNumber(String num) {
        //一共八位
        char[] nums = new char[8];
        int p = 7, now = 7;
        char[] c = num.toCharArray();
        /**
         * 从右到左逐个字符解析 通过p控制万位,pp控制万以下的单位,now记录当前单位
         */
