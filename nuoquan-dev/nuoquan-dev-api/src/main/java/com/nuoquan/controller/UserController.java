...
public class UserController extends BasicController {
    ...
    /**
     * Description 上传文件到 fastdfs 文件服务器 的实例方法。 PS: fastdfs 为非结构化储存，暂不使用，本方法仅供参考。
     *
     * @author jerrio
     */
    @Deprecated
    ...
    public JSONResult getWxUserInfo(String code, String iv, String encryptedData, String nickname, String faceImg)
        ...
        log.info("wechat get openId return: " + res);
        WxRes wxRes = JsonUtils.jsonToPojo(res, WxRes.class);
        User user = new User();
        String id = wxRes.getOpenid();
        // 2. 获取用户 openId
        UserVO userExist = userService.getUserVOByUniAppOpenId(id);
        if (userExist != null) {
            //判断该用户是否被禁言，如果被禁言则判断该用户的禁言日期是否到达，如果到达则使该用户状态变为1（正常）
            userService.judgeUserState(userExist.getId());
            return JSONResult.ok(userExist);
        }
        ...
        // 保存 openId
        user.setUniAppOpenId(wxRes.getOpenid());
    ...
    public UserVO wxLogin(@RequestBody User userData) throws Exception {
        ...
        if (!isIdExist) {
            ...
            user.setFansNum(0);
            user.setCreateDate(new Date());
            user.setUniAppOpenId(userData.getUniAppOpenId());
            userVO = userService.saveUserDirectly(user);
        } else {
            // 3.2 查询信息
            ...
            userVO = userService.getUserById(userData.getId());
            // 如果 openId 为空，则更新数据库
            if (StringUtils.isEmpty(userVO.getUniAppOpenId())) {
                userVO.setUniAppOpenId(userData.getUniAppOpenId());
                userService.updateUniAppOpenId(userVO.getId(), userData.getUniAppOpenId());
            }
            //判断该用户是否被禁言，如果被禁言则判断该用户的禁言日期是否到达，如果到达则使该用户状态变为1（正常）
    ...
    @ApiImplicitParams({ @ApiImplicitParam(name = "email", required = true, dataType = "String", paramType = "form"),
    ...
    public JSONResult searchUserYang(String searchText, Integer page, String userId) throws Exception {
    ...
    public JSONResult addUser(@ApiParam(value = "file", required = true) MultipartFile faceImg, String userId,
