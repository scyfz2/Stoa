...
public class EventsCalendarController extends BasicController {
    ...
    public JSONResult queryEventsCalendarByDate(String userId, Integer page, Integer pageSize, Integer targetDate,
            Integer faculty, Integer degree) {
    ...
    public JSONResult uploadEvent(String userId, String title, String venue, Integer date, String time,
            FacultyType faculty, DegreeType degree) throws Exception {
        ...
        if (faculty.getContent() == "all") {
            ...
            for (int f = 1; f < faculty.getType(); f++) {
                ...
                if (degree.getContent() == "all") {
                    ...
                    for (int d = 1; d < degree.getType(); d++) {
                        eventsCalendar = eventsCalendarService.insert(eventsCalendar, title, venue, date, time, f, d,
                                isLegal);
                        eventId = eventsCalendarService.saveEvent(eventsCalendar);
                    }
                } else {
                    // 如果学历选择不为全部
                    eventsCalendar = eventsCalendarService.insert(eventsCalendar, title, venue, date, time, f,
                            degree.getType(), isLegal);
                    eventId = eventsCalendarService.saveEvent(eventsCalendar);
                }
            }
        } else {
            ...
            if (degree.getContent() == "all") {
                // 遍历所有学历种类
                for (int d = 1; d < degree.getType(); d++) {
                    eventsCalendar = eventsCalendarService.insert(eventsCalendar, title, venue, date, time,
                            faculty.getType(), d, isLegal);
                    eventId = eventsCalendarService.saveEvent(eventsCalendar);
                }
            } else {
                // 学历选择不为全部
                eventsCalendar = eventsCalendarService.insert(eventsCalendar, title, venue, date, time,
                        faculty.getType(), degree.getType(), isLegal);
                eventId = eventsCalendarService.saveEvent(eventsCalendar);
            }
        }
