...
public class ArticleController extends BasicController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "selectedPart", value = "选择的分区", required = false, dataType = "String", paramType = "form") })
    @PostMapping("/queryArticles")
    public JSONResult queryArticles(Integer page, Integer pageSize, Integer queryType, Integer orderType, String userId,
                                    String selectedTag, String selectedPart)
            throws Exception {
        ...
        PagedResult result = articleService.queryArticle(page, pageSize, queryType, userId, selectedTag, selectedPart);
     ...
     * @throws Exception
     */
    @ApiOperation(value = "查询分区文章", notes = "查询分区文章的接口")
    @ApiImplicitParams({
            // userId 查询用户和文章的点赞关系
            // dataType 为 String, 应该改为 Integer
            @ApiImplicitParam(name = "userId", value = "操作者id", required = true, dataType = "String", paramType = "form"),
            ...
            @ApiImplicitParam(name = "orderType", value = "排列方式", required = true, dataType = "Integer", paramType = "form"),
            @ApiImplicitParam(name = "selectedPart", value = "选择的分区", required = false, dataType = "String", paramType = "form") })
    @PostMapping("/queryArticlesByPart")
    public JSONResult queryArticlesByPart(Integer page, Integer pageSize, Integer queryType, Integer orderType,
                                          String userId, String selectedTag, String selectedPart)
            throws Exception {
        if (page == null) {
            page = 1;
        }
     ...
     * @throws Exception
     */
    @ApiOperation(value = "查询置顶文章", notes = "查询置顶文章的接口")
    @ApiImplicitParams({
            // userId 查询用户和文章的点赞关系
            // dataType 为 String, 应该改为 Integer
            @ApiImplicitParam(name = "userId", value = "操作者id", required = true, dataType = "String", paramType = "form"),
            ...
            @ApiImplicitParam(name = "orderType", value = "排列方式", required = true, dataType = "Integer", paramType = "form"),
            @ApiImplicitParam(name = "selectedPart", value = "选择的分区", required = false, dataType = "String", paramType = "form") })
    @PostMapping("/queryArticlesByTop")
    public JSONResult queryArticlesByTop(Integer page, Integer pageSize, Integer queryType, Integer orderType,
                                         String userId, String selectedTag, String selectedPart)
            throws Exception {
        // 补充追加置顶文章
        List<ArticleVO> articleVOS = articleService.queryArticleByPart(selectedPart, userId);
        if (CollectionUtil.isEmpty(articleVOS)) {
            return JSONResult.ok(articleVOS);
        }
        Collections.sort(articleVOS, Comparator
                .comparing(ArticleVO::getTop, Comparator.nullsLast(Comparator.naturalOrder())) // 根据top字段正序，处理可能为空的情况
                .thenComparing(
                        Comparator.comparing(ArticleVO::getTopOrder, Comparator.nullsLast(Comparator.reverseOrder()))) // 根据topOrder字段倒序，处理可能为空的情况
        );
        return JSONResult.ok(articleVOS);
    }
    ...
    public JSONResult upload(String userId, String articleTag, String articleTitle, String articleContent)
        ...
        if (userMapper.selectByPrimaryKey(userId).getState() == 1) {
            ...
            // 默认非置顶 非加精
            article.setEssence(new Byte("0"));
            article.setTop(new Byte("0"));
            article.setCreateDate(new Date());
            // 检测内容是否非法
            if (weChatService.msgSecCheck(articleTitle) && weChatService.msgSecCheck(articleTag)
            ...
            // tag 保存
            tagsService.checkAndSaveTags(articleTag);
    ...
    public JSONResult uploadArticleImg(String userId, String articleId, String order,
    ...
    @ApiImplicitParams({ @ApiImplicitParam(name = "keyWord", value = "搜索关键字", required = true, dataType = "String") })
    public JSONResult getTagsList(String keyWord) {
        return JSONResult.ok(tagsService.getTagsList(keyWord));
    }
