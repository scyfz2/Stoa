...
public class SocialController extends BasicController {
    ...
    public JSONResult userLike(String userId, PostType targetType, String targetId) throws Exception {
        ...
        if (!userId.equals(authorId)) {
            notifyRemindService.insert(userId, NotifyRemindService.SenderAction.LIKE, sourceId, targetType, targetId,
                    authorId);
        }
    ...
    public JSONResult saveComment(String fromUserId, String toUserId, // 更方便判断是否为自己点赞，以及查询对方昵称
                                  PostType targetType, String targetId, String comment, String underCommentId)
            throws Exception {
        // 当用户状态为1时才能发表文章
        ...
        if (user.getState() == 1) {
            ...
            // 插入评论
            String sourceId = socialService.insertComment(fromUserId, toUserId, targetType, targetId, comment,
                    underCommentId);
    ...
    @ApiImplicitParams({ @ApiImplicitParam(name = "page", required = false, dataType = "Integer", paramType = "form"),
    ...
    public JSONResult getMainComments(Integer page, Integer pageSize, Integer type, PostType targetType,
        ...
        PagedResult list = socialService.getMainComments(page, pageSize, type, targetType, targetId, userId);
    ...
    public JSONResult fDeleteComment(String commentId, String userId, String targetId, PostType targetType)
            throws Exception {
        if (StringUtils.isBlank(userId) || StringUtils.isEmpty(userId) || StringUtils.isBlank(commentId)
                || StringUtils.isEmpty(commentId)) {
