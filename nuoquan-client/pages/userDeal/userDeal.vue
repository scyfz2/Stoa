<template>
	<view class="container_ud">
		...
		<scroll-view scroll-y="true" class="introduction-content">
			<!-- 请不要尝试展开以下标签 -Guetta -->
			<text style="line-height: 16px;font-size: 14px;word-break: break-all;white-space: pre-line;text-overflow: ellipsis;color: #3d3d3d;">
				特别提示
				...
				本协议约定聚像网络与用户之间关于“GT Channel”微信小程序服务（以下简称“服务”）的权利义务。“用户”是指注册、登录、使用本服务的个人。本协议可由聚像网络随时更新，更新后的协议条款一旦公布即代替原来的协议条款，恕不再另行通知，用户可在本网站查阅最新版协议条款。在聚像网络修改协议条款后，如果用户不接受修改后的条款，请立即停止使用聚像网络提供的服务，用户继续使用聚像网络提供的服务将被视为接受修改后的协议。
				一、帐号注册
				1、用户在使用本服务前需要注册一个“GT Channel”帐号。“GT Channel”帐号应当与用户的微信账号绑定注册，请用户使用尚未与“GT Channel”帐号绑定的微信账号，以及未被聚像网络根据本协议封禁的微信账号注册“GT Channel”帐号。聚像网络可以根据用户需求或产品需要对帐号注册和绑定的方式进行变更，而无须事先通知用户。
				2、鉴于“GT Channel”帐号的绑定注册方式，您同意聚像网络在注册时将使用您提供的微信账号及自动提取您的手机设备识别码等信息用于注册。
				...
				6、聚像网络重视对未成年人个人信息的保护。聚像网络将依赖用户提供的个人信息判断用户是否为未成年人。任何18岁以下的未成年人注册帐号或使用本服务应事先取得家长或其法定监护人（以下简称"监护人"）的书面同意。除根据法律法规的规定及有权机关的指示披露外，聚像网络不会使用或向任何第三方透露未成年人的聊天记录及其他个人信息。除本协议约定的例外情形外，未经监护人事先同意，聚像网络不会使用或向任何第三方透露未成年人的个人信息。
				...
				2、用户不得利用“GT Channel”帐号或本服务制作、上载、复制、发布、传播如下法律、法规和政策禁止的内容：
				(1) 反对宪法所确定的基本原则的；
				...
				3、用户不得利用“GT Channel”帐号或本服务制作、上载、复制、发布、传播如下干扰“GT Channel”正常运营，以及侵犯其他用户或第三方合法权益的内容：
				(1) 含有任何性或性暗示的；
				(2) 含有辱骂、恐吓、威胁内容的；
				...
				2、用户不得利用“GT Channel”帐号或本服务进行如下行为：
				(1) 提交、发布虚假信息，或盗用他人头像或资料，冒充、利用他人名义的；
				(2) 强制、诱导其他用户关注、点击链接页面或分享信息的；
				(3) 虚构事实、隐瞒真相以误导、欺骗他人的；
				(4) 利用技术手段批量建立虚假帐号的；
				(5) 利用“GT Channel”帐号或本服务从事任何违法犯罪活动的；
				(6) 制作、发布与以上行为相关的方法、工具，或对此类方法、工具进行运营或传播，无论这些行为是否为商业目的；
				(7) 其他违反法律法规规定、侵犯其他用户合法权益、干扰“GT Channel”正常运营或聚像网络未明示授权的行为。
				3、用户须对利用“GT Channel”帐号或本服务传送信息的真实性、合法性、无害性、准确性、有效性等全权负责，与用户所传播的信息相关的任何法律责任由用户自行承担，与聚像网络无关。如因此给聚像网络或第三方造成损害的，用户应当依法予以赔偿。
				...
				六、账户管理
				1、“GT Channel”帐号的所有权归聚像网络所有，用户同意微信授权并与用户微信账号绑定后，获得“GT Channel”帐号的使用权，该使用权仅属于初始申请注册人，禁止赠与、借用、租用、转让或售卖。聚像网络因经营需要，有权回收用户的“GT Channel”帐号。
				...
				4、用户应遵守本协议的各项条款，正确、适当地使用本服务，如因用户违反本协议中的任何条款，聚像网络在通知用户后有权依据协议中断或终止对违约用户“GT Channel”帐号提供服务。同时，聚像网络保留在任何时候收回“GT Channel”帐号、用户名的权利。
				5、如用户注册“GT Channel”帐号后一年不登录，通知用户后，聚像网络可以收回该帐号，以免造成资源浪费，由此造成的不利后果由用户自行承担。
				七、数据储存
				...
				八、风险承担
				1、用户理解并同意，“GT Channel”仅为用户提供信息分享、传送及获取的平台，用户必须为自己注册帐号下的一切行为负责，包括用户所传送的任何内容以及由此产生的任何后果。用户应对“GT Channel”及本服务中的内容自行加以判断，并承担因使用内容而引起的所有风险，包括因对内容的正确性、完整性或实用性的依赖而产生的风险。聚像网络无法且不会对因用户行为而导致的任何损失或损害承担责任。
				如果用户发现任何人违反本协议约定或以其他不当的方式使用本服务，请立即向聚像网络举报或投诉，举报或投诉电话为18757439170，聚像网络将依本协议约定进行处理。
				...
				5、用户理解并确认，聚像网络需要定期或不定期地对“GT Channel”平台或相关的设备进行检修或者维护，如因此类情况而造成服务在合理时间内的中断，聚像网络无需为此承担任何责任，但聚像网络应事先进行通告。
				...
				8、在任何情况下，聚像网络均不对任何间接性、后果性、惩罚性、偶然性、特殊性或刑罚性的损害，包括因用户使用“GT Channel”或本服务而遭受的利润损失，承担责任（即使聚像网络已被告知该等损失的可能性亦然）。尽管本协议中可能含有相悖的规定，聚像网络对用户承担的全部责任，无论因何原因或何种行为方式，始终不超过用户因使用聚像网络提供的服务而支付给聚像网络的费用(如有)。
				十二、服务的变更、中断、终止
